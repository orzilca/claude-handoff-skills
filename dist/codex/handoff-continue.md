...
2. **Argument that is a bare name** → substring-match it (case-insensitive) against filenames in `.codex/handoff/`. If multiple match, pick the **newest** by modification time. If none match, tell the user, list what *is* in the dir, and stop.
3. **No argument** → load the **newest** file in `.codex/handoff/` by modification time. If the dir is missing or empty, tell the user there's no handoff to resume and stop.

Find the newest file with a single command, e.g. `ls -t .codex/handoff/*.md`.

## Then

1. **Read the resolved file fully** with the file-reading tool.
2. **Confirm which handoff loaded** — one line, e.g. `Resuming handoff: .codex/handoff/20260701-143022-parkalot-cron.md`.
