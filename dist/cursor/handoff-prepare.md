...
This skill only **writes** the file. Resuming is separate: the user starts a fresh session (a new chat), then runs /handoff-continue. Never claim the context was cleared — only the user can do that.
...
3. **Write the file** to `.cursor/handoff/{timestamp}-{slug}.md` in the format below.
   - `mkdir -p .cursor/handoff/` first. Timestamp: `date +%Y%m%d-%H%M%S`. Slug: 2-4 distinctive kebab-case words (/handoff-continue matches on it).
