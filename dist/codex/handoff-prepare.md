...
This skill only **writes** the handoff. Resuming is a separate step: the user starts a fresh session (/new), then runs /handoff-continue. Do not claim the context was cleared — you cannot clear it; only the user can.
...
3. **Write the file** to `.codex/handoff/{timestamp}-{slug}.md` using the template below.
   ...
   - Ensure the dir exists: `mkdir -p .codex/handoff/`.
