...
Find the newest file with a single command (no pipes into the tool call — write to `.claude/tmp/*.sh` and run it if you need a pipeline per Bash hygiene rules), e.g. `ls -t .claude/tmp/handoff/*.md`.

## Then

1. **Read the resolved file fully** with the Read tool.
